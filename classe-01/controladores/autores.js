...
const cadastrarAutor = async (req, res) => {
    ...
    try {
        ...
        if (autor.rowCount === 0) return res.status(400).json('Não foi possivel cadastrar o autor');
...
const atualizarAutor = async (req, res) => {
    ...
    try {
        ...
        if (autorAtualizado.rowCount === 0) return res.status(404).json('Não foi possível atualizar o autor');
...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const obrasAutor = await conexao.query('select * from livros where autor_id = $1', [id]);
        if (obrasAutor.rowCount !== 0) return res.status(401).json("Autor possui obras cadastradas");

        const autorExcluido = await conexao.query('delete from autores where id = $1', [id]);
        if (autorExcluido.rowCount === 0) return res.status(404).json('Não foi possível excluir o autor');
